...
- Assignee (Select dropdown -- fetch team members from `/users` endpoint)
...
5. Create task with client + policy linked -- appears in task list
6. Edit a renewal task -- title/description/dueDate/priority fields disabled
7. Edit a manual task -- all fields editable
  </verify>
  <done>Kanban board renders 4 status columns with drag-and-drop between columns. Task cards show title, priority, client, due date, type badge. Task form dialog supports create and edit with client/policy linking, assignee selection. Renewal tasks enforce read-only content fields in edit mode.</done>
</task>
...
7. Renewal tasks: visually distinguished (badge), content fields read-only in edit
8. Filters: search, status, priority, type all work
9. Pagination works in table view
</verification>
