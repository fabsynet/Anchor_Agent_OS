...
5. `getPremiumIncome(tenantId)`: Calculate current month total, YTD total, and trend vs previous month.
   ...
   - Three parallel queries: currentMonth (createdAt >= startOfMonth(now)), YTD (createdAt >= startOfYear(now)), previousMonth (createdAt between start and end of previous month).
...
- `GET /dashboard/premium-income` returns `{ currentMonth, ytd, previousMonth, trendPercentage }`
  </verify>
  <done>Dashboard API serves all 5 data sources: summary counts, upcoming renewals, overdue tasks, recent activity feed, and premium income with trend percentage. All queries use manual tenantId for count/aggregate operations.</done>
</task>
...
1. `sendDailyDigestForAllTenants()`:
   ...
   - For each user, gather their digest data (overdue tasks, renewal milestones for next 7 days).
...
3. `getDigestDataForUser(tenantId, userId)`:
   ...
   - Renewal milestones: tasks where tenantId matches, type = 'renewal', status = 'todo', dueDate within next 7 days. Include policy and client relations.
...
If RESEND_API_KEY is set, can manually trigger the digest by calling the service method or hitting a test endpoint.
  ...
  <done>NotificationsModule sends daily digest at 8 AM Toronto time. Digest includes overdue tasks and upcoming renewal milestones. Empty digests are skipped. Users with digestOptOut=true are excluded. Email uses React Email template rendered to HTML via @react-email/render, sent via Resend SDK.</done>
</task>
...
4. Premium income trend calculation handles zero previous month gracefully
5. Notification scheduler registered with correct cron expression and timezone
6. Digest skips users with digestOptOut=true
7. Empty digests (no overdue tasks, no renewal milestones) are not sent
</verification>
