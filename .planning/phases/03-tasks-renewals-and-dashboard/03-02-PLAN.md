...
must_haves:
  ...
  artifacts:
    - path: "apps/api/src/tasks/tasks.controller.ts"
      provides: "Task CRUD endpoints: GET /tasks, POST /tasks, GET /tasks/:id, PATCH /tasks/:id, DELETE /tasks/:id"
...
- `DELETE /tasks/:id` -- returns success
  ...
  <done>Task CRUD API is fully functional: create, list (with search/filter/pagination), get, update, delete. Renewal tasks reject content edits (title/description/dueDate changes). Activity events logged for task_created, task_completed, task_status_changed. ScheduleModule.forRoot() registered in AppModule.</done>
</task>
...
2. In `create()` method: After the $transaction succeeds, if the created policy has an endDate, call `this.renewalsService.generateTasksForPolicy(result)` (pass the full policy object including tenant relation data). Note: the result from $transaction may not include relations, so construct the policy-with-tenant object or query it fresh.

3. In `update()` method: After the update succeeds, check two conditions:
   - If `dto.endDate !== undefined` and endDate actually changed from existing: call `this.renewalsService.regenerateRenewalTasks(updatedPolicy)` (delete pending + regenerate).
...
3. Update the policy endDate to 60 days from now
4. Verify old renewal tasks deleted and new ones created
5. Change policy status to 'cancelled'
6. Verify all pending renewal tasks deleted
  </verify>
  <done>Renewal engine is operational: daily cron generates renewal tasks idempotently across all tenants, policy create/update/cancel triggers renewal task lifecycle management. No duplicate tasks, no orphaned tasks. Three milestones (60/30/7 days) with escalating priority (medium/high/urgent).</done>
</task>
...
3. Renewal tasks auto-generate when policy with endDate is created
4. Renewal tasks regenerate when policy endDate changes
5. Renewal tasks delete when policy is cancelled
6. Renewal task content cannot be edited (only status changes allowed)
7. Idempotency: running renewal generation twice doesn't create duplicates
</verification>
